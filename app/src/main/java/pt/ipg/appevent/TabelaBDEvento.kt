...
class TabelaBDEvento(db: SQLiteDatabase) : TabelaBD(db, NOME) {
    override fun cria() {
        db.execSQL("CREATE TABLE $nome (${BaseColumns._ID} INTEGER PRIMARY KEY AUTOINCREMENT, $nome_evento TEXT NOT NULL,"+
                " $data TEXT NOT NULL, $descricao TEXT NOT NULL,"+
                " $id_localidade INTEGER NOT NULL, FOREIGN KEY ($id_localidade) REFERENCES ${TabelaBDLocalidade.NOME}(${BaseColumns._ID}) ON DELETE RESTRICT) ")
    }
